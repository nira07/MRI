...
class MRIViewer(QWidget):
    def __init__(self, nii_path):
        super().__init__()
        self.setWindowTitle('Axial MRI Viewer')
        self.resize(600, 700)
        ...
        self.label = QLabel(f"Axial Slice: {self.slice_index}")
        layout = QVBoxLayout()
    ...
    def plot_slice(self):
        ...
        ax.set_title(f"Axial Slice {self.slice_index}")
        ax.axis('off')
        self.canvas.draw()

    def update_plot(self, value):
        self.slice_index = value
        self.label.setText(f"Axial Slice: {self.slice_index}")
        self.plot_slice()
