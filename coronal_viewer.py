...
class MRIViewer(QWidget):
    def __init__(self, nii_path):
        super().__init__()
        self.setWindowTitle('Coronal MRI Viewer')
        self.resize(600, 700)
        self.img = nib.load(nii_path)
        self.data = self.img.get_fdata()
        self.slice_index = self.data.shape[1] // 2
        ...
        self.label = QLabel(f"Coronal Slice: {self.slice_index}")
        layout = QVBoxLayout()
    ...
    def plot_slice(self):
        ...
        ax.set_title(f"Coronal Slice {self.slice_index}")
        ax.axis('off')
        self.canvas.draw()

    def update_plot(self, value):
        self.slice_index = value
        self.label.setText(f"Coronal Slice: {self.slice_index}")
        self.plot_slice()
